...
- Keep `rou3/compiler` imports confined to `src/compiler.ts` / `src/compiler/*` so they stay out of runtime bundles. `src/compiler.ts` is a thin barrel over `src/compiler/*` (split by area: `compile.ts` entrypoints, `options.ts` types/input, `runtime-rules.ts`, `serialize.ts`, `matcher-export.ts`, `pre-merge.ts`, `binding.ts`) — the public export surface stays identical.
