...
- There is **no default `cache` / `proxy` handler**: both are opt-in subpath exports so their deps stay out of bundles that don't use them (`cache`→ocache; `proxy`→h3's `proxyRequest`, in `src/proxy.ts` / `h3-rules/proxy`). `createRouteRulesMatcher` throws when rules use `cache`/`swr` or `proxy` with no matching entry in the handler registry (explicit `handlers: { cache: undefined }` / `{ proxy: undefined }` opts into data-only). The compiler sources both from their subpath (the `DEFAULT_RUNTIME_RULES` literal in `src/compiler/runtime-rules.ts`), so compiled bundles pull each rule's dep only when that rule exists. `redirect` stays a core registry handler — it shares `resolveRuleTarget` (ufo) with `proxy` but not `proxyRequest`.
